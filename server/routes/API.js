...
router.get("/",(req,res) => {
    ...
    let iso_code = req.query.iso_code ? req.query.iso_code.replace(/\s/g, '').toUpperCase() : undefined
    ...
    let formatted_query = {
        iso_code: iso_code ? {$in: iso_code} : undefined,
        year: year ? year_type_range ? {$gt: year_from, $lt: year_to} : {$in: year} : undefined,
    }
    Object.keys(formatted_query).forEach(key => formatted_query[key] === undefined ? delete formatted_query[key] : {})
